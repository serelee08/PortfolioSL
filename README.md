<div align="right">
  <a href="README.md">🇰🇷 한국어</a> &nbsp;|&nbsp; <a href="README_en.md">🇺🇸 English</a>
</div>

# 이지애 | 소프트웨어 엔지니어

> 명확한 API 설계와 AI 기능 통합으로 안정적인 시스템을 구축합니다.

🌐 [포트폴리오 사이트](https://serelee08.github.io/PortfolioSL) · 📧 serelee08@gmail.com · 💻 [github.com/serelee08](https://github.com/serelee08)

---

## 기술 스택

`Java` `Spring Boot` `Python` `Django` `C#` `MyBatis` `React` `TypeScript`  
`MySQL` `MariaDB` `Oracle` `AWS EC2` `OAuth2` `Spring Security`

---

## 프로젝트

### 바티칸 국제 행사 시스템 개발 
`2026.03 – 2026.05` · 실무 프로젝트

- 대형 국제 행사 관련 Oracle DB 테이블 설계 (9개 테이블)
- Java 기반 서비스 로직 구현
- C# 기반 화면 개발 및 DevExpress 출력물 구현
- 병원 ERP 간단 유지보수 업무

---

### EVLink – AI 기반 전기차 충전기 공유 플랫폼
`2025.08 – 2025.09` · 팀 프로젝트

- Django 기반 Gemini AI 대화형 서비스 REST API 구현
- LSTM 감성분석 모델 설계 및 백엔드 서비스 연동
- OAuth2 기반 인증 구조 설계 및 AWS EC2 배포

🔗 [포트폴리오 보기](https://drive.google.com/file/d/1WHECWwoLdS4O4JYRylPk6vEio3aPRkjs/view?usp=sharing)

---

### Walley – 통계 기반 자산 관리 플랫폼
`2025.05 – 2025.07` · 팀 프로젝트

- Spring Boot 기반 RESTful API 설계 및 수입·지출 처리 로직 구현
- MyBatis 기반 Oracle DB 연동 및 데이터 모델링
- 영수증 이미지 업로드 및 카테고리별 조회 API 개발
- React + TypeScript 프론트엔드 연동

🔗 [포트폴리오 보기](https://drive.google.com/file/d/1Nqu_IvhZziX_aOnuyx6c47OsQPEBOHj2/view?usp=share_link)

---

## 수상

🏆 프로젝트 우수상 · 🏅 특모범상 (출석률 100%) – 한국 ICT 인재개발원

---

## 자격증

- 정보처리기능사 – 한국산업인력공단 (필기 합격 2026.06 · 실기 예정 2026.08)

---

## 교육

- 클라우드 기반 AI 풀스택 개발 과정 – 한국 ICT 인재개발원 (2025.03 – 2025.09)
- IBM AI Engineering Professional – Coursera (2025.12 – 현재)

---

## 개발 방향

요청 처리 흐름과 파이프라인 설계를 중심으로 외부 서비스 및 AI 기능을 백엔드에 안정적으로 통합하는 개발을 지향합니다.  
잘 설계된 구조가 팀의 생산성과 서비스 안정성을 동시에 높인다고 믿습니다.
